  ...
  return (
    <div className='container mx-auto p-6'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8'>
        {games.map(game => (
          <Link className='group relative bg-gray-900 p-5 rounded-xl shadow-lg transform transition hover:scale-105 hover:shadow-xl'
           key={game.id}
           href={`/games/${game.category}/${game.slug}`}>
            <div className='relative'>
              <img className='w-full h-52 object-cover rounded-lg group-hover:opacity-80 transition'
              src={game.image} 
              alt={game.title} />
